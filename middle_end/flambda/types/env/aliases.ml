...
module Alias_set = struct
  ...
  (** Create the set of aliases of some particular element (not seen here).
      ...
      element. *)
  let create ~canonical_element ~coercion_from_canonical_to_element ~alias_names =
    Simple.pattern_match canonical_element
      ...
      ~name:(fun canonical_name ->
        let const = None in
        let names =
          Name.Map.add canonical_name coercion_from_canonical_to_element alias_names
        in
        { const; names })
  ...
  | Is_canonical canonical_element ->
    let alias_names =
      Aliases_of_canonical_element.all
        (get_aliases_of_canonical_element t ~canonical_element)
    in
    Alias_set.create ~canonical_element ~coercion_from_canonical_to_element:Coercion.id ~alias_names
