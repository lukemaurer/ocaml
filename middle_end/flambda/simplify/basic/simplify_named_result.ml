...
let have_simplified_to_single_term dacc bindable_let_bound defining_expr
  ...
  { dacc;
    descr = Single_term { let_bound = bindable_let_bound;
                          simplified_defining_expr = defining_expr;
                          original_defining_expr };
  }

let have_lifted_set_of_closures dacc bound_vars_to_symbols =
  (* The benefit of lifting the set of closure is added in
     [simplify_named]. *)
  { dacc;
...
let bindings_to_place_in_any_order t =
  ...
  | Multiple_bindings_to_symbols bound_vars_to_symbols ->
    Var_in_binding_pos.Map.fold (fun bound_var symbol bindings ->
        let let_bound = Bindable_let_bound.singleton bound_var in
        let simplified_defining_expr =
          Simple.symbol symbol
          |> Flambda.Named.create_simple
