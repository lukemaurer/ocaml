...
let simplify_direct_partial_application ~simplify_expr dacc apply
  ...
  let wrapper_taking_remaining_args, dacc, code_id, code =
    ...
    let open struct
      ...
      (* CR lmaurer: Also allow coerced symbols. Would require putting any
         depth variables in the closure, which is desirable but currently not
         possible.

         Note that this workaround has the effect of turning the callee from a
         symbol into a variable. Fortunately, the reconstituted [Apply_expr]
         will retain the original call kind, so it will remain a direct call. *)
      type applied_value =
